...
const insertTodo = async (todo) => {
    const [headers] = await mysql_config_1.default.query(insertTodoQuery, [todo.id, todo.title, todo.description, todo.state, todo.priority]);
    console.log(headers);
    return { id: headers.insertId, ...todo };
};
exports.insertTodo = insertTodo;
const deleteTodo = async (id) => {
    const [headers] = await mysql_config_1.default.query(deleteTodoQuery, [id]);
    console.log(headers);
};
exports.deleteTodo = deleteTodo;
const updateTodo = async (todo) => {
    // *** This method automatically, creates a connection to the pool. And releases it after the query!
    const [headers, _] = await mysql_config_1.default.query(updateTodoQuery, [todo.title, todo.description, todo.state, todo.priority, todo.id]);
    console.log(headers);
    return { id: headers.insertId, ...todo };
};
...
const getAllTodos = async () => {
    ...
    rows.forEach((element) => {
        const parsedModel = todo_z_1.TODOSchema.safeParse(element);
